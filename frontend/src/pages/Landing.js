...
const Landing = () => {
  ...
  return (
    <div className="min-h-screen bg-jolshaa-surface dark:bg-[#0b1326] font-sans">
      {/* ── NAVBAR ─────────────────────────────────────────── */}
      <nav className="sticky top-0 z-50 bg-jolshaa-surface/80 dark:bg-[#0b1326]/80 backdrop-blur-xl shadow-sm dark:border-b dark:border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <span className="font-display text-xl font-bold text-jolshaa-teal dark:text-white">Jolshaa</span>
            <div className="hidden sm:flex items-center gap-3">
              <Link to="/login" className="px-4 py-2 text-sm font-medium text-jolshaa-teal hover:text-jolshaa-teal-container dark:text-neutral-300 dark:hover:text-white transition-colors">
                Log In
              </Link>
      ...
      {/* ── HERO ──────────────────────────────────────────── */}
      <section className="relative overflow-hidden bg-jolshaa-surface dark:bg-[#0b1326]">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            {/* Left — Copy */}
            <div className="space-y-6">
              <h1 className="font-display text-4xl sm:text-5xl font-bold text-jolshaa-on-surface dark:text-white leading-[1.2] tracking-tight">
                Where Every Voice{' '}
              ...
              <p className="text-lg text-jolshaa-on-surface-variant dark:text-neutral-400 max-w-lg leading-relaxed">
              ...
              <div className="flex items-center gap-4 pt-4">
                <div className="flex -space-x-3">
                  {['#00685f', '#4e45d5', '#924628', '#22c55e', '#3b82f6'].map((color, i) => (
                    <div key={i} className="w-10 h-10 rounded-full border-2 border-jolshaa-surface dark:border-[#0b1326] flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: color, zIndex: 5 - i }}>
                      {['A', 'M', 'S', 'K', 'R'][i]}
                    </div>
                  ))}
                </div>
                <span className="text-sm font-medium text-jolshaa-on-surface-variant dark:text-neutral-400">Join 10,000+ members</span>
              </div>
            </div>
                ...
                <div className="rounded-2xl overflow-hidden relative aspect-video lg:aspect-square">
                  ...
                  <div className="absolute inset-0 p-4 pt-6">
                    <div className="space-y-3">
                      ...
                      {/* Post cards */}
                      {[1, 2].map((i) => (
                        <div key={i} className="bg-white/60 dark:bg-white/5 backdrop-blur rounded-xl p-3 space-y-2 shadow-ambient">
      ...
      {/* ── STATS BAR ──────────────────────────────────────── */}
      <section className="relative py-12 bg-jolshaa-surface-container-low dark:bg-[#0f172a] border-y border-jolshaa-outline-variant/30 dark:border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((stat) => (
              <div key={stat.label} className="text-center">
                <p className="font-display text-3xl sm:text-4xl font-bold text-jolshaa-teal">{stat.value}</p>
                <p className="text-sm text-jolshaa-on-surface-variant dark:text-neutral-400 mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── FEATURES ──────────────────────────────────────── */}
      <section id="features" className="py-20 sm:py-28 bg-jolshaa-surface dark:bg-transparent">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-jolshaa-on-surface dark:text-white">
              Everything You Need to{' '}
              <span className="text-jolshaa-teal">Connect & Create</span>
            </h2>
            <p className="mt-4 text-lg text-jolshaa-on-surface-variant dark:text-neutral-400">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, i) => (
              <div key={i} className="group relative p-6 bg-jolshaa-surface-container-lowest dark:bg-white/[0.03] rounded-2xl shadow-ambient hover:shadow-ambient-hover transition-all duration-300 hover:-translate-y-1">
                ...
                <h3 className="text-lg font-bold text-jolshaa-on-surface dark:text-white mb-2">{feature.title}</h3>
                <p className="text-sm text-jolshaa-on-surface-variant dark:text-neutral-400 leading-relaxed">{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── TESTIMONIALS ──────────────────────────────────── */}
      <section className="py-20 sm:py-28 bg-jolshaa-surface-container-low dark:bg-[#0f172a]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-jolshaa-on-surface dark:text-white">
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div key={i} className="relative p-6 bg-jolshaa-surface-container-lowest dark:bg-white/[0.03] rounded-2xl shadow-ambient">
                ...
                <p className="text-jolshaa-on-surface-variant dark:text-neutral-300 text-sm leading-relaxed mb-6">"{t.text}"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-jolshaa-on-surface dark:text-white">{t.name}</p>
                    <p className="text-xs text-jolshaa-on-surface-variant dark:text-neutral-400">{t.role}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="md:hidden">
            <div className="relative p-6 bg-jolshaa-surface-container-lowest dark:bg-white/[0.03] rounded-2xl shadow-ambient min-h-[280px]">
              <div className="flex items-center gap-1 mb-4">
                {[...Array(5)].map((_, j) => (
                  <svg key={j} className="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                ))}
              </div>
              <p className="text-jolshaa-on-surface-variant dark:text-neutral-300 text-sm leading-relaxed mb-6">"{testimonials[currentTestimonial].text}"</p>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-jolshaa-on-surface dark:text-white">{testimonials[currentTestimonial].name}</p>
                  <p className="text-xs text-jolshaa-on-surface-variant dark:text-neutral-400">{testimonials[currentTestimonial].role}</p>
                </div>
              </div>
      ...
      {/* ── CTA SECTION ────────────────────────────────────── */}
      <section className="py-20 sm:py-28 bg-jolshaa-surface dark:bg-transparent">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* ── FOOTER ─────────────────────────────────────────── */}
      <footer className="py-12 bg-jolshaa-surface-container-lowest dark:bg-[#0a0f1e] border-t border-jolshaa-outline-variant/30 dark:border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2 mb-4">
                <span className="font-display text-lg font-bold text-jolshaa-teal dark:text-white">Jolshaa</span>
              </div>
              <p className="text-sm text-jolshaa-on-surface-variant dark:text-neutral-400 leading-relaxed">
                The social platform for creators, communities, and meaningful connections.
              </p>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-jolshaa-on-surface dark:text-white mb-4">Product</h4>
              <ul className="space-y-2">
                {['Features', 'Marketplace', 'Creator Hub', 'Groups', 'Events'].map((item) => (
                  <li key={item}><a href="#features" className="text-sm text-jolshaa-on-surface-variant dark:text-neutral-400 hover:text-jolshaa-teal dark:hover:text-white transition-colors">{item}</a></li>
                ))}
              </ul>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-jolshaa-on-surface dark:text-white mb-4">Company</h4>
              <ul className="space-y-2">
                {['About', 'Blog', 'Careers', 'Press', 'Contact'].map((item) => (
                  <li key={item}><a href="#" className="text-sm text-jolshaa-on-surface-variant dark:text-neutral-400 hover:text-jolshaa-teal dark:hover:text-white transition-colors">{item}</a></li>
                ))}
              </ul>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-jolshaa-on-surface dark:text-white mb-4">Legal</h4>
              <ul className="space-y-2">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between pt-8 border-t border-jolshaa-outline-variant/30 dark:border-white/5">
            <p className="text-sm text-jolshaa-on-surface-variant dark:text-neutral-400">&copy; {new Date().getFullYear()} Jolshaa. All rights reserved.</p>
            <div className="flex items-center gap-4 mt-4 sm:mt-0">
              {[
                ...
                <a key={social.label} href="#" className="text-jolshaa-on-surface-variant dark:text-neutral-400 hover:text-jolshaa-teal transition-colors" aria-label={social.label}>
