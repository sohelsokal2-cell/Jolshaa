  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Create Subscription Tier" size="md">
      <div className="p-5 space-y-4">
        <div>
          <label className="block text-sm font-medium text-jolshaa-on-surface mb-1">Tier Name</label>
          <input
            type="text"
            value={name}
        ...
        <div>
          <label className="block text-sm font-medium text-jolshaa-on-surface mb-1">Price (BDT/month)</label>
          <input
            type="number"
            min="0"
            value={price}
        ...
        <div>
          <label className="block text-sm font-medium text-jolshaa-on-surface mb-1">Badge Emoji</label>
          <input
            type="text"
            value={badge}
        ...
        <div>
          <label className="block text-sm font-medium text-jolshaa-on-surface mb-1">Perks</label>
            ...
            {perks.map((perk, i) => (
              <div key={i} className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-jolshaa-outline rounded-lg bg-jolshaa-surface-container-highest text-jolshaa-on-surface text-sm focus:ring-2 focus:ring-jolshaa-teal focus:border-transparent"
                />
                {perks.length > 1 && (
                  <button onClick={() => removePerk(i)} className="px-2 text-jolshaa-on-surface-variant hover:text-red-500">✕</button>
                )}
              </div>
            ))}
            <button onClick={addPerk} className="text-sm text-jolshaa-teal hover:text-jolshaa-teal-container font-medium">
              + Add Perk
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={onClose} className="flex-1 py-2.5 border border-jolshaa-outline-variant rounded-lg text-sm font-medium hover:bg-jolshaa-surface-container-high text-jolshaa-on-surface">
            Cancel
          </button>
          <button
  ...
  if (loading) {
    return (
      <div className="space-y-3">
        {[1, 2].map(i => <div key={i} className="h-32 bg-jolshaa-surface-container-high rounded-xl animate-pulse" />)}
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display font-semibold text-jolshaa-on-surface">
      ...
      {tiers.length === 0 ? (
        <p className="text-jolshaa-on-surface-variant text-center py-8">No subscription tiers yet</p>
      ) : (
        <div className="space-y-3">
          {tiers.map(tier => (
            <div key={tier._id} className="bg-jolshaa-surface-container-lowest border border-jolshaa-outline-variant rounded-xl p-4">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    {tier.badge && <span className="text-lg">{tier.badge}</span>}
                    <h4 className="font-display font-semibold text-jolshaa-on-surface">{tier.name}</h4>
                  </div>
                  <p className="text-xl font-bold text-jolshaa-teal mt-1">৳{tier.price}/mo</p>
                  {tier.perks?.length > 0 && (
                    <ul className="mt-2 space-y-1">
                      {tier.perks.map((perk, i) => (
                        <li key={i} className="text-sm text-jolshaa-on-surface-variant flex items-center gap-1">
                          <span className="text-green-500">✓</span> {perk}
                        </li>
                      ))}
                    </ul>
                  )}
                  <p className="text-xs text-jolshaa-on-surface-variant mt-2">{tier.subscriberCount} subscribers</p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  {!isOwner && (
                    <button
                      ...
                      className="px-4 py-2 bg-jolshaa-teal text-white rounded-lg text-sm font-medium hover:bg-jolshaa-teal-container disabled:opacity-50 transition-colors"
                    >
                  ...
                  {isOwner && (
                    <button
                      ...
                      className="px-3 py-2 text-sm text-red-600 hover:text-red-700 border border-red-200 rounded-lg"
                    >
                      Delete
                    </button>
                  )}
