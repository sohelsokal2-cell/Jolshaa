...
const FactCheckBadge = ({ post, onUpdate }) => {
  ...
  const badgeConfig = {
    ...
    false: {
      ...
      label: 'এই পোস্টটি গুজব হতে পারে — সতর্ক থাকুন',
    },
    misleading: {
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-red-700 dark:text-red-300">
                এই পোস্টটি গুজব হতে পারে — সতর্ক থাকুন
              </p>
              {totalVotes > 0 && (
    ...
    <>
      <div className="ml-[52px] mt-1">
        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${config.bg} ${config.text} border ${config.border}`}>
          ...
          {config.label}
          {totalVotes > 0 && (
            <span className="font-normal opacity-70">({totalVotes} ভোট)</span>
          )}
          <button
            onClick={() => setShowModal(true)}
