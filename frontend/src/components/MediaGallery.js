    ...
    <div>
        ...
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex-1 py-2 text-[10px] font-medium transition ${
              activeTab === tab.id ? 'text-blue-600 border-b-2 border-blue-600' : 'text-jolshaa-on-surface-variant/60 hover:text-jolshaa-on-surface-variant'
            }`}>
            {tab.label}
          </button>
        ))}
      ...
      <div className="p-3">
        {loading ? (
          ...
          photos.length === 0 ? (
            <p className="text-center text-jolshaa-on-surface-variant/60 text-sm py-8">No photos shared yet</p>
          ) : (
          ...
          videos.length === 0 ? (
            <p className="text-center text-jolshaa-on-surface-variant/60 text-sm py-8">No videos shared yet</p>
          ) : (
            <div className="grid grid-cols-2 gap-1">
              {videos.map(msg => (
                <div key={msg._id} className="aspect-video bg-jolshaa-surface-container-high rounded-lg overflow-hidden cursor-pointer"
          ...
          files.length === 0 ? (
            <p className="text-center text-jolshaa-on-surface-variant/60 text-sm py-8">No files shared yet</p>
          ) : (
            <div className="space-y-2">
              {files.map(msg => (
                <a key={msg._id} href={msg.media} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 p-3 bg-jolshaa-surface-container-low rounded-lg hover:bg-jolshaa-surface-container transition">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-jolshaa-on-surface-variant/60">
                      {msg.fileSize ? `${(msg.fileSize / 1024).toFixed(1)} KB` : ''}
                      {msg.sender?.name && ` · ${msg.sender.name}`}
                    </p>
                  </div>
                  <svg className="w-4 h-4 text-jolshaa-on-surface-variant/60 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          links.length === 0 ? (
            <p className="text-center text-jolshaa-on-surface-variant/60 text-sm py-8">No links shared yet</p>
          ) : (
            <div className="space-y-2">
              {links.map(msg => (
                <a key={msg._id} href={msg.linkPreview?.url || msg.text} target="_blank" rel="noopener noreferrer"
                  className="block p-3 bg-jolshaa-surface-container-low rounded-lg hover:bg-jolshaa-surface-container transition">
                  ...
                  {msg.linkPreview?.title && <p className="text-xs text-jolshaa-on-surface-variant mt-0.5">{msg.linkPreview.title}</p>}
                  <p className="text-xs text-jolshaa-on-surface-variant/60 mt-0.5">Shared by {msg.sender?.name}</p>
                </a>
              ))}
            </div>
          )
