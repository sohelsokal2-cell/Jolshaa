...
const URGENCY_COLORS = {
  ...
  within_hours: 'bg-yellow-100 text-yellow-700 border-yellow-200',
  within_days: 'bg-green-100 text-green-700 border-green-200',
};
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-display font-bold text-jolshaa-on-surface text-sm leading-snug">
              {request.title}
            </h3>
        ...
        {/* Description preview */}
        <p className={`text-sm text-jolshaa-on-surface-variant mb-3 ${expanded ? '' : 'line-clamp-2'}`}>
          {request.description}
        </p>
        {request.description.length > 100 && (
          <button
            onClick={() => setExpanded(!expanded)}
            className="text-xs text-jolshaa-teal hover:underline mb-3"
          >
        ...
        {/* Helper count */}
        <div className="flex items-center gap-2 text-xs text-jolshaa-on-surface-variant mb-3">
          ...
          <span className="text-jolshaa-outline-variant">·</span>
        ...
        {offered ? (
          ...
          <div className="space-y-2">
            <textarea
              ...
              className="w-full border border-jolshaa-outline rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-500 bg-jolshaa-surface-container-highest resize-none"
              rows={2}
            />
            <button
              onClick={handleOffer}
